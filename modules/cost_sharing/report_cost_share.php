...
<head>
    ...
    <title data-en="Generate Report Cost Share" data-am="የወጪ መጋራት ሪፖርት ያመንጩ">Generate Report Cost Share</title>
...
<body>
    <div class="dashboard-container">
        ...
        <div class="layout-body">
            ...
            <div class="main-content">
                <div class="top-bar">
                    <h2 data-en="Report Cost Share (Cost Sharing Pro)" data-am="የወጪ መጋራት ሪፖርት (የወጪ መጋራት ባለሙያ)">Report
                        Cost Share (Cost Sharing Pro)</h2>
                </div>
                        ...
                        <div class="filter-grid">
                            ...
                            <div style="grid-column: span 2;">
                                ...
                                <input type="text" name="student_id" placeholder="Search by ID..."
                                    ...
                                    data-am-placeholder="በመለያ ይፈልጉ..." value="<?php echo $_GET['student_id'] ?? ''; ?>">
                            </div>
                    ...
                    <table class="table-list">
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                ...
                                $sex_map = ['Male' => 'ወንድ', 'Female' => 'ሴት'];
                                foreach ($reports as $r):
                                    $dept_am = $academic_translations[$r['dept_name']] ?? $r['dept_name'];
                                    $status_key = strtolower($r['status']);
                                    $status_am = $status_map[$status_key] ?? $r['status'];
                                    $sex_am = $sex_map[$r['sex']] ?? $r['sex'];
                                    ?>
                                    <tr>
                                        <td><?php echo htmlspecialchars($r['student_id']); ?></td>
                                        <td><?php echo htmlspecialchars($r['first_name'] . ' ' . $r['last_name']); ?></td>
                                        <td><span data-en="<?php echo htmlspecialchars($r['sex']); ?>"
                                                data-am="<?php echo $sex_am; ?>"><?php echo htmlspecialchars($r['sex']); ?></span>
                                        </td>
                                        <td><span data-en="<?php echo htmlspecialchars($r['dept_name']); ?>"
                                                data-am="<?php echo htmlspecialchars($dept_am); ?>"><?php echo htmlspecialchars($r['dept_name']); ?></span>
                                        </td>
                                        ...
                                        <td>
                                            ...
                                            if (!empty($r['is_transfer_out'])) {
                                                ...
                                                $disp_am = $status_am ?? ($status_map[strtolower($r['status'])] ?? ucfirst($r['status']));
                                            }
                                            ?>
                                            <span class="status-badge status-<?php echo $disp_class; ?>"
                                                data-en="<?php echo $disp_en; ?>" data-am="<?php echo $disp_am; ?>">
                                                <?php echo $disp_en; ?>
                                            </span>
                                        </td>
    ...
    <script>
        // Use the registrar's helper since it's the same logic. 
        // We need to point to the correct path. It is in ../../api/get_dropdown_options.php
        // Relative to modules/cost_sharing/ is ../../api/
            ...
            if (college) {
                fetch(`../../api/get_dropdown_options.php?action=get_departments&college=${encodeURIComponent(college)}`)
                    ...
                    .then(data => {
                        ...
                        // Re-apply language settings for new elements
