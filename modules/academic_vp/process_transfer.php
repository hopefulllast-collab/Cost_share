...
$vp_name = $_SESSION['name'] ?? 'Academic Vice President';
$error = '';
$msg = '';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action_type'])) {
    ...
    if ($action_type == 'upload_stamp') {
        ...
        if (strpos($doc['clearance_file'], 'signed_') !== 0) {
            ...
            $stmt = $pdo->prepare("UPDATE official_transcript SET status = 'Forwarded', description = 'Document Digitally/Manually Stamped and Forwarded' WHERE id = ?");
            if ($stmt->execute([$id])) {
                $_SESSION['flash_success'] = "<span data-en='Document forwarded to Registrar successfully.' data-am='ሰነዱን ወደ ሬጅስትራር በተሳካ ሁኔታ ልከናል።'>Document forwarded to Registrar successfully.</span>";
                header("Location: view_requested_document.php");
                exit();
            } else {
            ...
            <div class="main-content">
                <div class="top-bar">
                    <h2 data-en="Transfer-Out Document Processing" data-am="የግቢ ዝውውር ሰነድ ማዘጋጀት">Transfer-Out Document
                        Processing</h2>
                    <a href="view_requested_document.php" class="btn-secondary"><i class="fas fa-arrow-left"></i> <span
                    ...
                    <div class="action-panel">
                        ...
                        <div class="action-box" style="background:#f8f9fa; border-color:#0056b3;">
                            ...
                            <p style="font-size:13px; color:#333;"
                                data-en="Once you have stamped or uploaded a new file, forward it to the Registrar."
                                data-am="ማህተም ካደረጉ ወይም አዲስ ፋይል ከጫኑ በኋላ ወደ ሬጅስትራር ያስተላልፉት።">Once you have stamped or
                                uploaded a new file, forward it to the Registrar.</p>
                            <form method="POST">
                                ...
                                <?php else: ?>
                                    <button type="submit" class="btn-primary" style="width:100%; margin-top:5px;">
                                        <i class="fas fa-share"></i> <span data-en="Forward to Registrar"
                                            data-am="ወደ ሬጅስትራር አስተላልፍ">Forward to Registrar</span>
                                    </button>
                                <?php endif; ?>
                            </form>
                        </div>
