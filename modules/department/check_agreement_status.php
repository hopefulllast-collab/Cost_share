...
checkAuth(['registrar', 'admin', 'academic_vp', 'cost_sharing_pro', 'department_head']);

$is_dept_head = ($_SESSION['role'] === 'department_head');
$forced_dept_id = null;
if ($is_dept_head) {
    ...
    $deptHead = $stmt->fetch();
    if ($deptHead) {
        $forced_dept_id = $deptHead['department_id'];
    } else {
        die("<div style='padding:40px; text-align:center;'>Oops! You are not assigned to any department.</div>");
    }
}
...
if ($is_dept_head && $forced_dept_id) {
    $dept_id = $forced_dept_id; // Department Head sees ONLY their department
}
$batch = $_GET['batch'] ?? '';
        ...
        <div class="layout-body">
            ...
            <div class="main-content">
                ...
                <form method="GET" class="filter-bar" style="display: flex; flex-wrap: wrap; gap: 10px;">
                    ...
                    <?php else: ?>
                        <!-- Hidden override for Dept Head to ensure JS loads batches properly if dependent on dropdown -->
                        <input type="hidden" name="department_id" id="deptSelect"
                            value="<?php echo htmlspecialchars($forced_dept_id); ?>">
                    <?php endif; ?>
                ...
                <?php if ($status_filter == 'all' || $status_filter == 'signed' || $status_filter == 'approved'): ?>
                    ...
                    <div class="card mt-20">
                        ...
                        <?php if (empty($display_students)): ?>
                            <p style="padding: 12px 15px; margin-bottom: 20px; border-radius: 6px; border-left: 4px solid #dc3545; background-color: #f8d7da; color: #721c24; font-weight: 500;"
                                data-en="No signed agreements found." data-am="ምንም የተፈረመ ስምምነት የለም።">No signed agreements found.
                            </p>
                        <?php else: ?>
                            <table class="table-styled">
                                <thead>
