...
// College Filter (Indirectly filters departments via UI, but strictly we can filter by joining if needed, 
// but usually dept_id is enough if UI ensures valid dept for college. 
// If dept is selected, that overrides college. If only college selected, we filter by college.)
...
if (!empty($_GET['year'])) {
    // Handle Custom vs Dropdown Year
    $year_val = $_GET['year'];
...
$sql = "SELECT s.student_id, s.first_name, s.last_name, s.sex, s.batch, s.status, d.name as dept_name,
        ...
        WHERE $where_sql
        GROUP BY s.student_id, s.first_name, s.last_name, s.sex, s.batch, s.status, d.name, st.academic_year, st.semester
...
// Status Map for Bilingual Display
$status_map = [
    'active' => 'ንቁ',
...
<head>
    ...
    <title data-en="Report Cost Share - Registrar" data-am="የወጪ መጋራት ሪፖርት - ሬጅስትራር">Report Cost Share - Registrar
    </title>
    ...
    <style>
        ...
        @media print {
            ...
            /* Hide all navigation, sidebars, topbars, forms, and buttons */
            ...
            .table-list { width: 100% !important; border-collapse: collapse; }
            .table-list th, .table-list td { border: 1px solid #000 !important; padding: 8px !important; text-align: left !important; color: #000 !important; }
            .layout-body { display: block !important; }
            .dashboard-container { display: block !important; }
            /* Ensure status badges print as text and not colored blobs (or preserve color if browser allows) */
            .status-badge { border: none !important; padding: 0 !important; background: transparent !important; color: #000 !important; font-weight: normal !important; }
        }
    </style>
</head>
    ...
    <div class="dashboard-container">
        ...
        <div class="layout-body">
            ...
            <div class="main-content">
                <div class="top-bar">
                    <h2 data-en="Report Cost Share (Registrar)" data-am="የወጪ መጋራት ሪፖርት (ሬጅስትራር)">Report Cost Share
                        (Registrar)</h2>
                </div>
                        ...
                        <div class="filter-grid">
                                ...
                                <select name="college" id="collegeSelect" onchange="loadDepartments()">
                                    ...
                                    ?>
                                        <option value="<?php echo htmlspecialchars($col); ?>" 
                                            ...
                                            <?php echo (isset($_GET['college']) && $_GET['college'] == $col) ? 'selected' : ''; ?>>
                            ...
                            <div>
                                ...
                                <select name="year" id="yearSelect" onchange="toggleCustomYear()">
                                    ...
                                    ?>
                                    <option value="Custom" <?php echo (isset($_GET['year']) && $_GET['year'] == 'Custom') ? 'selected' : ''; ?> data-en="Custom request..." data-am="ብጁ ጥያቄ...">Custom...
                                    </option>
                                </select>
                            ...
                            <div style="grid-column: span 2;">
                                ...
                                <input data-en="Student ID" data-am="የተማሪ መለያ" type="text" name="student_id"
                                    value="<?php echo $_GET['student_id'] ?? ''; ?>">
                            </div>

                            <!-- Search/Print Buttons -->
                            <div style="grid-column: span 2; display: flex; gap: 12px; align-items: flex-start; justify-content: flex-start;">
                                <button type="submit" class="btn-primary" style="padding: 8px 24px; font-size: 0.9rem; align-self: flex-end;">
                                    <i class="fas fa-search"></i> <span data-en="Search" data-am="ፈልግ">Search</span>
                                </button>
                                <button type="button" class="btn-secondary" style="padding: 8px 24px; font-size: 0.9rem; background: #6c757d; color: white; border: none; align-self: flex-end;" onclick="printReport()">
                                    <i class="fas fa-print"></i> <span data-en="Print Document" data-am="ሰነድ አትም">Print Document</span>
                                </button>
                            </div>
                        </div> 
                    </form>
                    ...
                    <table class="table-list">
                        ...
                        <tbody>
                            <?php if (empty($reports)): ?>
                                <tr>
                                    <td colspan="8" data-en="No records found." data-am="ምንም መዝገቦች አልተገኙም።">No records
                                        found.</td>
                                </tr>
                            <?php else: ?>
                                <?php foreach ($reports as $r): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <span
                                                data-en="<?php echo $r['sex'] == 'M' ? 'Male' : ($r['sex'] == 'F' ? 'Female' : $r['sex']); ?>"
                                                data-am="<?php echo $r['sex'] == 'M' ? 'ወንድ' : ($r['sex'] == 'F' ? 'ሴት' : $r['sex']); ?>">
                                        ...
                                        <td>
                                            <?php 
                                            $d_en = $r['dept_name'];
                                            $d_am = $academic_translations[$d_en] ?? $d_en;
                                            ?>
                                            <span data-en="<?php echo htmlspecialchars($d_en); ?>" data-am="<?php echo htmlspecialchars($d_am); ?>">
                                                <?php echo htmlspecialchars($d_en); ?>
                                            </span>
                                        </td>
                ...
                fetch(`../../api/get_dropdown_options.php?action=get_departments&college=${encodeURIComponent(college)}`)
                    ...
                    .then(data => {
                        data.forEach(dept => {
                            ...
                            const deptNameEn = dept.name; // Use display_name if available
                            const deptNameAm = academicTranslations[deptNameEn] || deptNameEn;
                            
                            option.textContent = deptNameEn;
